...
- Auto-trigger end-to-end requires `AGENDA_QUERY_DETECTOR_ENABLED=true` plus a live WS conversation. Still NOT browser-verified — the auto hook lives in `stt_ws_session.py:_persist_event` (the live WebSocket path), which the upload path does NOT exercise. Verifying it needs the fake-mic harness (`lct_app/tests/e2e/live-recording-stream.spec.ts`) with the flag on. Smoke-tested with synthetic finalized segments only.
