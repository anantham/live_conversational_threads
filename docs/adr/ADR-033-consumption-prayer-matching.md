...
- **IndrasNet base URL must be loopback when co-located.** On a host where LCT and IndrasNet run together (the dev box: `asus-strix-scar`), `INDRASNET_BASE_URL` must point at `http://127.0.0.1:7777`, NOT this host's own Tailscale IP. The consumption client's `httpx.AsyncClient` hairpin-stalls past its 5s timeout when connecting to the self Tailscale IP on Windows (`curl` tolerates it, async httpx does not). The code default (`100.81.65.74:7777`) is correct only for split-host deployments; set the env override per co-located machine. See `lct_python_backend/.env`.
...
- **Manual end-to-end browser-verified 2026-05-27** (Playwright, real auth on). Drove the full UX: uploaded a 24s fixture so the live transcript pane mounts, made a real DOM selection inside it, confirmed the toolbar renders with the 50-contact picker, clicked "go", watched the chip transition `looking up…` → IndrasNet response. **This pass caught a shipped bug:** `consumptionApi.js` called raw `fetch()` without the auth header, so the manual path 401'd under any auth-enforced backend — the prior "verified" claim was against a path that never exercised the apiClient auth layer. Fixed in commit `27e3391` (route through `apiFetch`). The earlier 2026-05-18 "verified against live IndrasNet" note was therefore incomplete; treat browser-verify, not unit tests, as the source of truth for this UI path.
- **Auto-trigger end-to-end browser-verified 2026-05-30** (Playwright, real backend with `AGENDA_QUERY_DETECTOR_ENABLED=true`). Drove the WS protocol directly from the page: opened `/ws/transcripts` with auth, sent `session_meta`, then synthesized a `transcript_final` event with a trigger phrase. Both resolution paths exercised:
  - **Name-grounded:** `"what's pending with Ignatius Milton"` → detector matched the name-expanded template `pending with ignatius milton` → runner resolved to the contact_id directly from the match → `consumption_match` WS event with `source: "auto"`, `match_source: "name-grounded"`.
  - **Participant fallback:** pre-set a participant via `PUT /api/conversations/{id}/participants`, then sent agnostic phrase `"what was pending again"` → detector matched default phrase, runner fell back to first non-self participant → same `consumption_match` event shape with `match_source: "default"`.
  The fake-mic harness (`lct_app/tests/e2e/live-recording-stream.spec.ts`) was NOT required — synthetic `transcript_final` events injected over the live WS protocol exercise the exact same `_persist_event` codepath. That harness is still the right choice for full STT-pipeline verification when needed.
