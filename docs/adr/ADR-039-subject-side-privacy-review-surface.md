---
Date: 2026-06-21
Status: **P2a backend BUILT + dual-reviewed (2026-06-22) — converged, both families GO, 84 tests green.** Design converged over three dual-family rounds (v1: 2 BLOCKING + 6 MAJOR architectural → v2: 1 BLOCKING + 3 MAJOR spec → v3: 2 MAJOR consistency, all fixed); the P2a implementation then converged over four more rounds (see *Implementation review history (P2a)*). Remaining: P2b frontend (`/subject-review/:token`), P2c the IndrasNet sender switch.
...
## Implementation review history (P2a)

The P2a backend (`subject_review_core.py` pure logic + `subject_review_api.py` endpoints + the `subject_review_bundles` migration + the middleware exemption) was built and dual-reviewed (codex `gpt-5.5` repo-grounded + grok-build second family) over four rounds. **Converged — both families GO with zero open findings; 84 tests green** (38 pure-core, 9 middleware-exemption incl. admin-mode, 37 Postgres state-machine + canaries, integration on the dev DB). Severity strictly decreasing each round.

- **R1:** codex NO-GO, 3 findings (all REAL, all fixed); grok GO. (1, BLOCKING) `/api/subject-review/import` was unauthenticated in the `ADMIN_AUTH_TOKEN`-only deploy mode → added to `_requires_admin_auth`. (2, BLOCKING) FastAPI's default 422 echoes the rejected `input` → a malformed import would echo the `callback_token`, and IndrasNet's client logs response bodies → the import now validates the body manually and returns a `{loc,type}`-only 422. (3, MINOR) GET/decisions did the 404/410 lifecycle check before the email gate (metadata oracle on the token) → reordered. grok also flagged a cosmetic concurrency nit (fail-UPDATE could clobber a just-relayed row) → guarded with `status <> 'relayed'`.
- **R2:** codex, 2 NOT-CLOSED refinements of R1 → fixed. (a) Pydantic's `extra_forbidden` puts the offending KEY in `loc`, so a secret smuggled as a JSON key still echoed → `_sanitize_loc()` masks any non-schema segment with `<extra>`. (b) the oracle fix only handled a *missing* bearer; a present-but-invalid bearer still hit the DB → verify the Google token BEFORE the DB lookup (`_authenticate_subject`), compare email after.
- **R3:** codex, 1 new Medium → fixed. An out-of-range `prayer_id` passed Pydantic (positive int) but the column is PG `INTEGER`; it would fail at INSERT, where SQLAlchemy's default error embeds the bound params (`callback_token` + item text) into a loggable exception → bounded `prayer_id` to `PG_INT4_MAX` (root cause) + wrapped the import INSERT so no parameterized DB error reaches a log.
- **R4:** grok GO + 2 defensive nits → fixed (guarded the import rollback; a `_sanitize_db_errors` decorator turns any `SQLAlchemyError` in GET/decisions into a sanitized 503 so the decisions/relay UPDATEs can't leak bound params either). codex GO with **zero findings** — re-confirmed every invariant (token never to browser/log, no `callback_url` SSRF, Google-gate-before-DB, immutable hash 409 in all states, allowlisted relay result + sanitized errors, post-relay scrub).

**Reusable lesson (consistent with P1):** grok GO'd in R1 while codex caught 3 real findings (two outside grok's inlined scope) — never trust a single-family GO on privacy-critical code; re-review each round's FIXES (R2/R3 each found a real residual of the prior fix). The recurring theme was *secrets-in-error-paths* (422 input echo → 422 key echo → DB-bound-param logging) — closed at each layer.
