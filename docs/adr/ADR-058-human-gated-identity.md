...
- Stable identity rides the **ECAPA 192-dim embedding space** the M5 STT server already emits (the same space IndrasNet/Strix store for cross-recording identity). NOT the unstable per-chunk `SPEAKER_NN` cluster ids.
- Diarization runs on the **slower post-flush refinement loop** over accumulated audio (per the owner's correct intuition + ADR-026), using the **M5 whisper diarizer** — verified correct (2 speakers, embeddings, 20× realtime) — rather than the MPS-degraded backend pyannote.
...
- This is a **feature chain**, deliberately sequenced (foundation → up): (a) surface IndrasNet confirmed signal + audit its existing review UI (#14); (b) turn on the M5-diarizer refinement loop (#3); (c) voice-match + confirm UI (#13); (d) the picker (#4) and voice-ID consume the confirmed set. (a)-interim is the shipped #97 proxy.
...
- Where the refinement diarizer runs — the M5 whisper diarizer (owner's choice, verified) vs Asus-CUDA pyannote; MPS is degraded and must be avoided.
