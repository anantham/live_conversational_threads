...
**Date:** 2026-06-07
**Status:** **IMPLEMENTED** (PR #73, merged to `main` 2026-06-21). Shipped on `main`: `lct_python_backend/services/privacy_boundary.py` (the shared `redact`/`restore`/`leak_verify` + `classify_engine_tier` + the sandboxed CLI door `spawn_external_cli` + `bootstrap_egress`), the byte-level leak-verify wired into `egress_chokepoint.py`, audio gates in `stt_openai_realtime.py` / `stt_provider_transports.py`, synthesis migrated onto the CLI door, the pinned consent map `services/privacy_boundary_map.json`, and proof tests (`test_privacy_boundary.py`, `test_egress_redaction_gate.py`, `test_no_raw_frontier_subprocess.py`).

> ⚠️ **The shipped design SUPERSEDES the body of this ADR below.** The build leak-verifies the *real outbound bytes* at the chokepoint instead of the hash-of-redacted-text **stamp** that the Decision/interface sections below still describe; so the stamp machinery, the "Resolutions of the five pre-review findings," and the Round-2/Round-3 review tables are now **historical record**, not the current design. A full body-rewrite to match the code is a deferred follow-up. **Owner decision recorded:** `owner_is_forbidden=false` (2026-06-21) — Aditya's own name is allowed out un-redacted; contacts stay pseudonymized. **Remaining work:** auto-derive the forbidden-name list from the IndrasNet contacts `external_llm_ok` table (today it is the hand-pinned `privacy_boundary_map.json`).
