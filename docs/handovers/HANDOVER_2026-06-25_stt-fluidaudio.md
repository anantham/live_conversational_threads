...
- **#4 picker** not re-verified live after the later backend restarts (verified earlier in session).
...
**Calibration:** skipped the Phase-1a cruft census + didn't re-verify live state at handover until the gate forced it — the [0015] lesson recurring. Re-verify live + run cruft census BEFORE declaring a handover complete, not after a prompt.
