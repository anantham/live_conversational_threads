...
// - Vertical swipes keep the current card; long cards retain native scrolling.
...
test("vertical native swipes leave short cards at the same abstraction", async ({ page }) => {
  await open(page);
  const card = page.getByTestId("mobile-deck-card");
