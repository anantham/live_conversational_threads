...
## 2026-09-24 — Observability alerts: target outages, writer telemetry, CPU, and Tempo retention (OPEN FOLLOW-UPS)

**Summary:** A historical Indras Net scrape outage also fired `IndrasNetErrorWriterMissing`, even though the writer thread was alive in all three captured freeze dumps. The missing-writer alert is now gated on `up{job="indrasnet"} == 1` in the isolated working branch, with a promtool regression for healthy scrape + missing writer, down target, and present writer. This prevents a secondary alert from masking the actionable target-down alert.

**Confirmed record loss:** Prometheus recorded three `exception/write_failed` outcomes from 2026-09-22 13:28:45–13:33:30 IST. Indras Net logs at `grimoire/IndrasNet/logs/web_server.launcher.log.1:14921-14922,14935-14938` report SQLite `database is locked`, followed by three `ERROR_PERSISTENCE_WRITE_FAILED` outcomes. `core/db/error_log.py:117-120` returns `None` after the DB exception; `core/db/error_writer.py:397-408` marks the record failed and does not retry that record. This is a separate real loss issue. A nearby index `std::bad_alloc` has no demonstrated causal link to these writes.

**Other alerts:** Grafana target-down windows on 2026-09-22 were 13:05:51–13:11:21 and 13:57–14:01 IST. Grafana logs show repeated SQLite busy/locked errors, including near the latter window and again on 2026-09-24 through 10:56 IST; timing overlap does not prove the locks caused every target outage. `SharedHostCpuSaturation` was a separate later event around 22:30–22:46 IST; a historical sample attributed substantial CPU to `ffmpeg.exe` PID 44136, but its owning job is no longer known. `TempoStorageFailure` matched two retention block deletion failures caused by Windows file locks (2026-09-23 01:38:49 and 2026-09-24 03:33:50 IST); later block deletion succeeded and the failed-flush counter was zero during the alert window, so the observed condition was retention cleanup, not a flush failure.

**Impact / blocker status:** Historical target availability and three structured exception records were affected. The rule/test correction is local to this branch and not deployed. No service was restarted or killed. Follow-up investigation is open for persistent Grafana SQLite contention, Indras Net writer retry/DB contention, FFmpeg task ownership, and Tempo Windows file-handle ownership.
