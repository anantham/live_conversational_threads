...
## Graph & UI Polish
- **2026-08-26 — macro tiers discard usable relationship topology (confirmed, blocking comprehension):** the production Aayush artifact has 167 nested semantic relations. Mapping descendants to their level-5 ancestors yields 159 within-arc relations and 8 cross-arc relations across 4 distinct arc pairs (`supports`, `clarifies`, `exemplifies`, `generalizes`), but the arc view renders 0 edges. The current tier-local projection drops cross-tier relations and applies the same thread/time swim-lane used for moments, producing a tall list rather than a relationship map. Recommended next step: choose and document a macro quotient-graph projection that rolls descendant edges up to visible ancestors, aggregates relation counts without inventing direction, and uses relation-aware layout; retain temporal lanes for moments/ideas. This is separate from the bounded readable-camera repair.
