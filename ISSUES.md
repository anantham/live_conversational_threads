...
- Obsidian canvas export gap for upload-generated conversations (2026-02-25): `POST /export/obsidian-canvas/{conversation_id}` returns 500 (`No nodes found in conversation data`) after `POST /api/import/process-file` + `POST /save_json/` because graph/chunk data is saved to JSON storage but `Node`/`Relationship` rows are not materialized for that conversation. Canonical export endpoint is blocked for this path unless it falls back to stored graph JSON.
